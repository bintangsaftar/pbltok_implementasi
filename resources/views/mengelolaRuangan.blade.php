...
<body>
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-transparent">
            <div class="container-fluid">
                <ul class="navbar-nav mx-auto">
                    <li class="nav-item">
                        <a class="nav-link text-light" href="/mengelola-ruangan" style="margin-left: 42px; font-family: roboto; font-size: 17px; font-weight: 600;">Ruangan</a>
                        <div class="RecRuangan" style="width: 160px; height: 5px; background: #3D9EDE; border-radius: 12px;"></div>
                    </li>
                    <li class="nav-item">
    ...
    <div class="container">
        <div class="MengelolaRuangan" style="color: black; font-size: 25px; font-family: Roboto; font-weight: 700; padding-top: 10px;">
            Mengelola Ruangan</div>
    </div>

    <table class="table table-bordered mx-auto" style="width: 85%; margin-top: 10px; border-color: gray; border-radius: 10px; overflow: hidden;">
        <thead>
            <tr style="background-color: #3D9EDE; color: white;">
                <th scope="col" style="width: 10%; text-align: center;">Kode Ruangan</th>
                <th scope="col" style="width: 10%; text-align: center;">Nama Ruangan</th>
                <th scope="col" style="width: 10%; text-align: center;">Luas Ruangan</th>
                <th scope="col" style="width: 10%; text-align: center;">Status Ruangan</th>
                <th scope="col" style="width: 15%; text-align: center;">Aksi</th>
            </tr>
        </thead>
        <tbody>
            @foreach($ruanganItems as $ruanganItem)
            <tr style="height: 80px" class="ruangan-row">
                <th scope="row" style="text-align: center; vertical-align: middle; font-weight: normal">{{ $ruanganItem->kode_ruangan }}</th>
                <th scope="row" style="text-align: center; vertical-align: middle; font-weight: normal">{{ $ruanganItem->nama_ruangan }}</th>
                <th scope="row" style="text-align: center; vertical-align: middle; font-weight: normal">{{ $ruanganItem->luas_ruangan }}</th>
                <th scope="row" style="text-align: center; vertical-align: middle; font-weight: normal">{{ $ruanganItem->status_ruangan }}</th>
                <td style="text-align: center; vertical-align: middle; align-items: center;">
                    <a href="{{ route('ubah-ruangan', ['idruangan' => $ruanganItem->idruangan]) }}" class="btn btn-warning" style="margin-right: 35px; font-weight: bold;">Ubah</a>
                    <button type="button" class="btn btn-danger delete-btn" data-id="{{ $ruanganItem->idruangan }}" style="font-weight: bold;">Hapus</button>
                </td>
            </tr>
            @endforeach
        </tbody>
    ...
    <div class="container" style="display: flex;">
        <div style="margin-right: 32%; display: flex;">
        ...
        <div>
            <a href="/tambah-ruangan">
                <button type="button" class="btn btn-primary" style="font-size: 12px; font-weight: bold;">Tambah Ruangan</button>
            </a>
        </div>
    </div>
    <script>
        ...
        function showData() {
            const rows = document.querySelectorAll('.ruangan-row');
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', () => {
                const idruangan = button.getAttribute('data-id');

                // Konfirmasi penghapusan
                if (confirm('Apakah Anda yakin ingin menghapus ruangan ini?')) {
                    // Kirim permintaan penghapusan ke server dengan ID ruangan yang sesuai
                    fetch(`/hapus-ruangan/${idruangan}`, {
                            method: 'DELETE',
                            headers: {
