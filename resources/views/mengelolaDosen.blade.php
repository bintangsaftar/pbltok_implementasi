...
<body>
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-transparent">
            <div class="container-fluid">
                <ul class="navbar-nav mx-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-light" href="/mengelola-dosen" style="margin-left: 49px; font-family:roboto; font-size: 17px; font-weight: 600;">Dosen</a>
                        <div class="RecDosen" style="width: 160px; height: 5px; background: #3D9EDE; border-radius: 12px;"></div>
                    </li>
                    <li class="nav-item">
    ...
    <div class="container">
        <div class="MengelolaDosen" style="color: black; font-size: 25px; font-family: Roboto; font-weight: 700; padding-top: 10px;">
            Mengelola Dosen</div>
    </div>
    ...
    <table class="table table-bordered mx-auto" style="width: 50%; margin-top: 10px; border-color: gray; border-radius: 10px; overflow: hidden;">
        <thead>
            <tr style="background-color: #3D9EDE; color: white;">
                <th scope="col" style="width: 35%; text-align: center;">NIP/NIK</th>
                <th scope="col" style="width: 35%; text-align: center;">Nama Dosen</th>
                <th scope="col" style="width: 30%; text-align: center;">Aksi</th>
            </tr>
        </thead>
        <tbody>
            @foreach($dosenItems as $dosenItem)
            <tr style="height: 80px" class="dosen-row">
                <th scope="row" style="text-align: center; vertical-align: middle; font-weight: normal">{{ $dosenItem->nipk }}</th>
                <th scope="row" style="text-align: center; vertical-align: middle; font-weight: normal">{{ $dosenItem->nama_dosen }}</th>
                <td style="text-align: center; vertical-align: middle; align-items: center;">
                    <a href="{{ route('ubah-dosen', ['iddosen' => $dosenItem->iddosen]) }}" class="btn btn-warning" style="margin-right: 35px; font-weight: bold;">Ubah</a>
                    <button type="button" class="btn btn-danger delete-btn" data-id="{{ $dosenItem->iddosen }}" style="font-weight: bold;">Hapus</button>
                </td>
            </tr>
            @endforeach
        </tbody>
    ...
    <div class="container" style="display: flex;">
        ...
        <div>
            <a href="/tambah-dosen">
                <button type="button" class="btn btn-primary" style="font-size: 12px; font-weight: bold;">Tambah Dosen</button>
            </a>
        </div>
    </div>
    <script>
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', () => {
                const iddosen = button.getAttribute('data-id');

                // Konfirmasi penghapusan
                if (confirm('Apakah Anda yakin ingin menghapus dosen ini?')) {
                    // Kirim permintaan penghapusan ke server dengan ID dosen yang sesuai
                    fetch(`/hapus-dosen/${iddosen}`, {
                            method: 'DELETE',
                            headers: {
