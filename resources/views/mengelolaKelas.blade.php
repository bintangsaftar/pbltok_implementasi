...
<body>
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-transparent">
            <div class="container-fluid">
                <ul class="navbar-nav mx-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-light" href="/mengelola-kelas" style="margin-left: 52px; font-family:roboto; font-size: 17px; font-weight: 600;">Kelas</a>
                        <div class="RecKelas" style="width: 160px; height: 5px; background: #3D9EDE; border-radius: 12px;"></div>
                    </li>
                    <li class="nav-item">
    ...
    <div class="container">
        <div class="MengelolaKelas" style="color: black; font-size: 25px; font-family: Roboto; font-weight: 700; padding-top: 10px;">
            Mengelola Kelas</div>
    </div>
    ...
    <table class="table table-bordered mx-auto" style="width: 50%; margin-top: 10px; border-color: gray; border-radius: 10px; overflow: hidden;">
        <thead>
            <tr style="background-color: #3D9EDE; color: white;">
                <th scope="col" style="width: 50%; text-align: center;">Kelas</th>
                <th scope="col" style="width: 50%; text-align: center;">Aksi</th>
            </tr>
        </thead>
        <tbody>
            @foreach($kelasItems as $kelasItem)
            <tr style="height: 80px" class="kelas-row">
                <th scope="row" style="text-align: center; vertical-align: middle; font-weight: normal">{{ $kelasItem->kelas }}</th>
                <td style="text-align: center; vertical-align: middle; align-items: center;">
                    <a href="{{ route('ubah-kelas', ['idkelas' => $kelasItem->idkelas]) }}" class="btn btn-warning" style="margin-right: 35px; font-weight: bold;">Ubah</a>
                    <button type="button" class="btn btn-danger delete-btn" data-id="{{ $kelasItem->idkelas }}" style="font-weight: bold;">Hapus</button>
                </td>
            </tr>
            @endforeach
        </tbody>
    ...
    <div class="container" style="display: flex;">
        ...
        <div>
            <a href="/tambah-kelas">
                <button type="button" class="btn btn-primary" style="font-size: 12px; font-weight: bold;">Tambah Kelas</button>
            </a>
        </div>
    </div>
    <script>
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                if (confirm('Apakah Anda yakin ingin menghapus kelas ini?')) {
                    // Kirim permintaan penghapusan ke server dengan ID kelas yang sesuai
                    fetch(`/hapus-kelas/${idkelas}`, {
                            method: 'DELETE',
                            headers: {
